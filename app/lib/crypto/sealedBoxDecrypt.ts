...
export async function unsealSEK(
  ...
  if (!sek) {
    const pubKeyPrefix = Array.from(recipientPublicKey.slice(0, 4))
      .map((b) => b.toString(16).padStart(2, '0'))
      .join('');
    throw new Error(
      `Failed to unseal SEK: authentication failed (sealedBytes=${sealed.length}, pubKeyPrefix=${pubKeyPrefix})`,
    );
  }
